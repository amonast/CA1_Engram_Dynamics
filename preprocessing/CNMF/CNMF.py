...
def main():
    """
    Run CNMF source extraction for a single animal/FOV/session via CLI arguments.
    ...
    CNMF(mc_path,TSeries,file_key,new_opts={'seedCNMF':False})
#%%
...
# %%  Load motion corrected data + parameters. Make sure there is only 2 .pkl  files in each directory from motion correction
def CNMF(mc_path,TSeries,plotting=True,save_results=True,new_opts=None):
    """
    Run CaImAn CNMF source extraction on a motion-corrected TSeries.

    Loads the motion-correction parameters/object for a TSeries, sets up
    default CNMF parameters (optionally overridden by `new_opts`), memory-maps
    the motion-corrected movie, runs patch-wise CNMF (optionally seeded from
    Cellpose segmentation masks), refits and evaluates the resulting
    components, computes dF/F, and optionally plots and saves the results.

    Parameters
    ----------
    mc_path : str
        Path to the folder containing the motion-correction `.mc.pkl` and
        `.mc_opts.pkl` files for this TSeries.
    TSeries : str
    ...
    Notes
    -----
    Assumes exactly one matching `*mc.pkl`/`*mc_opts.pkl` pair exists per
    TSeries in `mc_path`. Saves outputs (contour plots, results `.hdf5`)
    under `{parent_of(TSeries)}/caiman_output/cnmf`.
    """
    animal_path = os.path.split(TSeries)[0]
    TSer = 'TSeries'+os.path.split(TSeries)[-1].split('TSeries', 1)[1].strip()
    opts_file = [os.path.join(mc_path,f) for f in os.listdir(mc_path) if (os.path.split(TSeries)[-1] in f)&('mc_opts.pkl' in f)]
    opts_file.sort()
    mc_file = [os.path.join(mc_path,f) for f in os.listdir(mc_path) if (os.path.split(TSeries)[-1] in f)&('mc.pkl' in f)]
    mc_file.sort()# load motion correction options and MC object

    #Set up parameters 
    #load in the parameters from motion correction
    with open(opts_file[-1], 'rb') as f:
        opts = pickle.load(f)
    try:
        path = opts.data['path']
    except:
    ...
    opts.change_params(cnmf_opts_dict,verbose=True)
...
#%% load in mc object if present; if it is it will get the filenames for CNMF from this mc pkl file
    try:
        with open(mc_file[-1], 'rb') as f:
            mc = pickle.load(f)
        border_to_0 = 0 if mc.border_nan == 'copy' else mc.border_to_0
        fnames = mc.mmap_file
        print('using mmap files from pkl file'+ mc_file[-1]+' list of files used:')
        if fnames[0].startswith('./'):
            fnames=[os.path.join(path,f[2:]) for f in fnames]
    except:
        print('No motion correction pkl file found. using list of mmap files present:')
        border_to_0 = 0 
        fnames = glob.glob(f"{path}/*.mmap")
    
    if not os.path.exists(fnames[0]):
        print('No mmaps file found. using list of mmap files present:')
        border_to_0 = 0 
        fnames = glob.glob(f"{path}/*.mmap")    
    fnames.sort()
    for i in fnames:
